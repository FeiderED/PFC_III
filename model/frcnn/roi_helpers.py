...
class ROIHelpers:
    """
    Clase para ayudar en la manipulación de regiones de interés (ROIs).
    """
    ...
    def set_overlap_thresh(self, new_overlap_thresh):
        """
        Establece el umbral de superposición.
        """
        self.overlap_thresh = new_overlap_thresh

    def calc_iou(self, R, data, class_mapping):
        """
        Calcula la intersección sobre unión (IoU) entre las ROIs y las GT (ground truth).

        Args:
            R: Region proposal network (RPN).
            data: Datos de entrada que contienen las GT.
            class_mapping: Mapeo de las clases a números enteros.

        Returns:
            X, Y1, Y2, IoUs: ROIs, clases codificadas en one-hot, coordenadas de regresión y IoUs.
        """
        bboxes = data['bboxes']
        ...
        for bbox_num, bbox in enumerate(bboxes):
            ...
            gta[bbox_num, 0] = int(round(bbox['x1'] * (new_width / float(width)) / rpn_stride))
            gta[bbox_num, 1] = int(round(bbox['x2'] * (new_width / float(width)) / rpn_stride))
            gta[bbox_num, 2] = int(round(bbox['y1'] * (new_height / float(height)) / rpn_stride))
            gta[bbox_num, 3] = int(round(bbox['y2'] * (new_height / float(height)) / rpn_stride))
        ...
        for ix in range(R.shape[0]):
            ...
            for bbox_num in range(len(bboxes)):
                curr_iou = Metrics.iou([gta[bbox_num, 0], gta[bbox_num, 2], gta[bbox_num, 1], gta[bbox_num, 3]],
                                        [x1, y1, x2, y2])
    ...
    def apply_regr(x, y, w, h, tx, ty, tw, th):
        """Aplica la regresión, calcula un rectángulo más ajustado."""
    ...
    def apply_regr_np(X, T):
        """Aplica regresión a un conjunto de ROIs."""
    ...
    def apply_non_max_suppression_fast(self, boxes, probs):
        """Aplica supresión de no máximos (NMS) para eliminar solapamientos redundantes."""
    ...
    def convert_rpn_to_roi(self, rpn_layer, regr_layer, use_regr=True):
        """Convierte las salidas del RPN en ROIs."""
        ...
        for anchor_size in anchor_sizes:
            for anchor_ratio in anchor_ratios:
                ...
                if use_regr:
                    A[:, :, :, curr_layer] = self.apply_regr_np(
                        A[:, :, :, curr_layer],
                        regr
                    )
